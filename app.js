...
ui.btnLoadEmbedded.onclick = async ()=>{
  try{
    ui.log.textContent="";
    RAW = await parseCSVUrl('./data/Sleep_health_and_lifestyle_dataset.csv');
    afterLoad();
...
function buildManualForm(featureCols, schema, pre){
  // динамически рисуем поля по признакам
  const form = ui.manualForm;
  form.innerHTML = '';
  // datalists для категорий
  const lists = {};
  ...
  // добавить все datalist в конец формы
...
ui.btnPredictManual.onclick = async ()=>{
  try{
    ...
    // собрать объект-строку r: {col:value}
    const r = {};
    for (const c of DATA.featureCols){
      const el = document.getElementById(`mf_${c}`);
      if (!el) continue;
      if (typeof el.value === 'string' && el.value.trim()==='' && el.type==='number'){
        r[c] = NaN;
    ...
    // превратить в вектор через тот же препроцессор
    const xi = DATA.pre.transformRow(r);
    const X = tf.tensor2d(xi, [1, DATA.inputDim]);
    const p = await MODEL.predict(X).array();
    X.dispose();

    const inv = invertMap(DATA.labelMap);
    const preds = p[0].map((v,i)=> ({name: inv[i], p: v}));
    ...
    ui.manualOut.innerHTML = `
      ...
      <div class="small muted" style="margin-top:6px">Не является медицинским заключением.</div>
    `;
  } catch(e){
